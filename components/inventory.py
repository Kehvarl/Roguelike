    ...
    def add_item(self, item):
        ...
        if len(self.items) >= self.capacity:
            ...
            if item.item.is_treasure:
                # TODO: Handle Items with a Treasure Value
                results.append({
                    'item_added': item,
                    'message': Message('You gained {0} Treasure'.format(item.treasure_value), libtcod.gold)
                })
                self.owner.treasure_value += item.treasure_value
            else:
                results.append({
                    'item_added': item,
