...
def cast_lightning(*args, **kwargs):
    ...
    if target:
        results.append({'consumed': True, 'target': target,
                        'message': Message(
                            'A lightning bolt strikes the {0} with a loud thunder, dealing {1} damage'.format(
                                target.name, damage),
                            libtcod.white)})
        results.extend(target.fighter.take_damage(damage))
    else:
        results.append({'consumed': False, 'message': Message('No enemy is close enough to strike', libtcod.red)})
...
def cast_confuse(*args, **kwargs):
    ...
    for entity in entities:
        ...
        results.append(
            {'consumed': False, 'message': Message('There is no valid target at that location.', libtcod.yellow)})
